...
namespace Dorado.Spider.Html
{
    ...
    public class PeekableInputStream : Stream
    {
        ...
        public int Peek(int depth)
        {
            ...
            if (this._peekBytes.Length <= depth)
            {
                byte[] temp = new byte[depth + 10];
                for (int i = 0; i < this._peekBytes.Length; i++)
                {
                    temp[i] = this._peekBytes[i];
                }
                this._peekBytes = temp;
            }

            // does more data need to be read?
            if (depth >= this._peekLength)
            {
                int offset = this._peekLength;
                int length = (depth - this._peekLength) + 1;
                int lengthRead = this._stream.Read(this._peekBytes, offset, length);
                ...
                this._peekLength = depth + 1;
            }

            return this._peekBytes[depth];
        }

        private byte Pop()
        {
            byte result = this._peekBytes[0];
            this._peekLength--;
            for (int i = 0; i < this._peekLength; i++)
            {
                this._peekBytes[i] = this._peekBytes[i + 1];
            }
