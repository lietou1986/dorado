...
namespace Dorado.Core.FastData
{
    ...
    public class Conn : IConn
    {
        ...
        {
            if (arr == null || arr.Count == 0) throw new CoreException("对不起，普通分页必须要指定排序条件！");
            StringBuilder sb = new StringBuilder();
        ...
        #region RowNumber分页查询 SelectRowNumber
        ...
        public DataArray SelectRowNumber(string sql)
        {
            ...
            {
                LoggerWrapper.Logger.Error("Conn.SelectRowNumber发生错误：" + sql, ex);
                throw ex;
            }
            finally
            {
        ...
        #endregion RowNumber分页查询 SelectRowNumber
