    ...
    public class DefaultVirtualPathProvider : IVirtualPathProvider
    {
        ...
        public virtual string ToAppRelative(string virtualPath)
        {
            ...
            try
            {
                ...
                {
                    LoggerWrapper.Logger.Info("Path '{0}' cannot be made app relative: Path returned ('{1}') is not app relative.", virtualPath, result);
                    return null;
                }
                return result;
            }
            catch (Exception e)
            {
                ...
                LoggerWrapper.Logger.Info(string.Format("Path '{0}' cannot be made app relative", virtualPath), e);
                return null;
            }
        }
                ...
                if (!string.IsNullOrEmpty(rootPrefix))
                {
                    ...
                    foreach (var term in terms)
                    {
                        if (term == "..")
                        {
                            if (depth == 0)
                            {
                                LoggerWrapper.Logger.Info("Path '{0}' cannot be made app relative: Too many '..'", virtualPath);
                                return true;
                            }
                            depth--;
                        }
        ...
        public virtual bool TryFileExists(string virtualPath)
        {
            ...
            {
                LoggerWrapper.Logger.Info(string.Format("File '{0}' can not be checked for existence. Assuming doesn't exist.", virtualPath), e);
                return false;
            }
        }
