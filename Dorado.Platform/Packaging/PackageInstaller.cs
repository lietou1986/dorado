...
namespace Dorado.Platform.Packaging
{
    public class PackageInstaller : IPackageInstaller
    {
        ...
        public PackageInfo Install(Stream packageStream, string location, string applicationPath)
        {
            ...
            {
                throw new CoreException(T("Admin.Packaging.StreamError"), ex);
            }
        ...
        protected PackageInfo InstallPackage(IPackage package, IPackageRepository packageRepository, string location, string applicationPath)
        {
            ...
            {
                throw new CoreException(T("Admin.Packaging.BackupError"), exception);
            }
            ...
            if (previousInstalled)
            {
                ...
                {
                    throw new CoreException(T("Admin.Packaging.UninstallError"), exception);
                }
            }

            var packageInfo = ExecuteInstall(package, packageRepository, location, applicationPath);

            // check if the new package is compatible with current DoradoPlatform version
            ...
            if (descriptor != null)
            {
                ...
                if (!PluginManager.IsAssumedCompatible(descriptor.MinAppVersion))
                {
                    ...
                    var msg = T("Admin.Packaging.IsIncompatible", PlatformVersion.CurrentFullVersion);
                    LoggerWrapper.Logger.Error(msg);
        ...
        public void Uninstall(string packageId, string applicationFolder)
        {
            ...
            {
                throw new CoreException(T("Admin.Packaging.NotFound", packageId));
            }
        ...
        private bool RestoreExtensionFolder(string extensionFolder, string extensionId)
        {
            ...
            if (source.Exists)
            {
                ...
                {
                    throw new CoreException(T("Admin.Packaging.TooManyBackups", tempPath));
                }
