...
{
    /// <summary>
    /// Summary description for ChangedFileListHandler
    /// </summary>
    public class ChangedFileListHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            context.Response.Write(GetChangedFileList(context, dir));
        }

        public string GetChangedFileList(HttpContext context, string dir)
        {
                ...
                int.TryParse(context.Request.Params["permission"], out permission))
            {
                ...
                if (serverEntity != null)
                {
                    ...
                    foreach (VwsDirectoryInfo file in filelist)
                    {
                        ...
                        {
                            var strChildNotes = GetChangedFileList(context, file.FullName);
                            if (strChildNotes != "[]")
                                sb.Append(
                                                    string.Format(
                                                        "{{\"id\":\"{0}\",\"name\":\"{1}\",\"pId\":\"{2}\",\"isParent\":{3},\"nocheck\":{4},\"hasDelPermission\":{5},\"checked\":{6}, \"isProce\":{7}, \"nodes\":{8}}},",
                                                        file.FullName.Replace("\\", "\\\\"),
                                                        file.Name + " (" + file.UpdateTime.ToLocalTime() + ")",
                                                        dir.Replace("\\", "\\\\"), file.IsFolder.ToString().ToLower(),
                                                        nocheck.ToString().ToLower(), lastHaveDelPermission.ToString().ToLower(),
                                                        haveRole.ToString().ToLower(), isProce.ToString().ToLower(), strChildNotes));
                        }
                    }
                    if (sb.Length > 1)
                    {
