...
(function ($) {
    ...
    //added by heyongdong
    //历遍子节点，设置checkbox
    function setSonNodeModifiedCheckBox(setting, treeNode, value) {
        if (!treeNode) return;

        if (treeNode != setting.root) {
            //treeNode[setting.checkedCol] = value;
            //treeNode.check_True_Full = true;
            //treeNode.check_False_Full = false;
            //setChkClass(setting,checkObj, treeNode);
        }

        if (!treeNode[setting.nodesCol]) return;
        for (var i = 0, l = treeNode[setting.nodesCol].length; i < l; i++) {
            var _treeNode = treeNode[setting.nodesCol][i];
            if (_treeNode && _treeNode.icon && _treeNode.icon.indexOf('edit.png') > -1) {
                _treeNode[setting.checkedCol] = value;
                var checkObj = $("#" + _treeNode.tId + IDMark_Check);
                setChkClass(setting, checkObj, _treeNode);
            }
            setSonNodeModifiedCheckBox(setting, _treeNode, value);
        }
        repairParentChkClassWithSelf(setting, treeNode);
    }
    ...
    //added by heyongdong 选中所有修改的选项
    function repairModifiedChk(setting, checked) {
        if (setting.checkable) {
            for (var i = 0; i < setting.root[setting.nodesCol].length; i++) {
                var treeNode = setting.root[setting.nodesCol][i];

                if (treeNode.icon && treeNode.icon.indexOf('edit.png') > -1) {
                    treeNode[setting.checkedCol] = checked;
                    var checkObj = $("#" + treeNode.tId + IDMark_Check);
                    setChkClass(setting, checkObj, treeNode);
                }
                setSonNodeModifiedCheckBox(setting, treeNode, checked);
            }
        }
    }
    ...
    function zTreePlugin() {
        return {
                ...
                if (st.checkEvent(this.setting)) {
                    ...
                    if (treeNode.parentNode) {
                        expandCollapseParentNode(this.setting, treeNode.parentNode, true, false, function () {
                            ...
                            //$("#" + treeNode.tId + IDMark_Check).focus().blur();
                            //repairChkClass(this.setting, treeNode);
                        });
                    } else {
                        $("#" + treeNode.tId + IDMark_Icon).focus().blur();
                        //$("#" + treeNode.tId + IDMark_Check).focus().blur();
            ...
            // added by  heyongdong
            checkModifiedNodes: function (checked) {
                repairModifiedChk(this.setting, checked);
            },
            ...
            setChkClass: function (obj, treeNode) {
                if (!treeNode) return;
                if (!obj) return;
                setChkClass(this.setting, obj, treeNode);
            }
        };
    }
})(jQuery);